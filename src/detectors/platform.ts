  ...
  async run(context) {
    ...
    for (const file of context.inventory.files) {
      if (/AndroidManifest\.xml$/.test(file.relativePath)) {
        ...
        for (const match of file.content.matchAll(backup)) {
          signals.push(createSignal({
      ...
      if (/Info\.plist$/.test(file.relativePath)) {
        ...
        for (const match of file.content.matchAll(ats)) {
          signals.push(createSignal({
      ...
      if (SOURCE_EXT.test(file.relativePath)) {
        ...
        for (const match of file.content.matchAll(webviewBridge)) {
          signals.push(createSignal({
        ...
        for (const match of file.content.matchAll(insecureHttp)) {
          signals.push(createSignal({
        ...
        for (const match of file.content.matchAll(asyncSensitive)) {
          signals.push(createSignal({
        ...
        for (const match of file.content.matchAll(flutterPreferences)) {
          signals.push(createSignal({
        ...
        for (const match of file.content.matchAll(flutterWebview)) {
          signals.push(createSignal({
        ...
        for (const match of file.content.matchAll(badCertificate)) {
          signals.push(createSignal({
    ...
    return {
      ...
      coverage: {
        ...
        engine: "aisec-native",
        status: relevant ? "complete" : "not_run",
        required: relevant,
        reason: relevant ? undefined : "No supported mobile project detected",
