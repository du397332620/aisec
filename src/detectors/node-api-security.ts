...
export async function runNodeApiSecurity(context: ScanContext): Promise<DetectorResult> {
  ...
  const reasons = [
    `${detected} route and authorization coverage is bounded static inference; package aliases and package-provided or mixin base classes, dynamic registrations outside the supported local-constant forEach form, framework extensions, dynamic module metadata and provider factories without a statically visible local class result, local call chains or inheritance beyond their four-edge traversal limits, filter helpers outside the direct-consumption/single-object-return form, dynamic/interpolated QueryBuilder predicates, computed parameter maps, unsupported ORM operators, Sequelize scopes, MongoDB aggregation pipelines, general policy control flow, complex ORM wrappers, and external policy engines may require manual review`,
