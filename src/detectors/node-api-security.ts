...
export async function runNodeApiSecurity(context: ScanContext): Promise<DetectorResult> {
  ...
  const reasons = [
    `${detected} route and authorization coverage is bounded static inference; package aliases and package-provided or mixin base classes, dynamic registrations outside the supported file-level const or one-hop directly imported relative-ESM table with at most two direct inline static filter/map transforms and inline forEach or direct synchronous for-of consumption, arbitrary collection transforms, framework extensions, forward references outside an official inline direct callback, async/branching/runtime dynamic module metadata, RouterModule registrations outside a direct official static @Module import with an eight-edge/256-entry route tree, ambiguous controller-module ownership, private/conflicting/unreachable providers, module graphs beyond eight edges or 256 entries, bootstrap wrappers or runtime root selection outside a directly invoked official NestFactory.create call, imperative global guards and application routing configuration outside a directly awaited same-container const application binding, provider factories without a statically visible local class result, local call chains or inheritance beyond their four-edge traversal limits, filter helpers outside the single-object-return/direct-or-one-const-consumption form, boolean policies outside the single-equality/direct-or-one-const-denial/one-wrapper form, dynamic/interpolated QueryBuilder predicates, computed parameter maps, unsupported ORM operators, Sequelize scopes, MongoDB aggregation pipelines, general policy control flow, complex ORM wrappers, and external policy engines may require manual review`,
    ...
    analysis.unresolvedProviderDependencies > 0
      ...
      : undefined,
    analysis.unresolvedBootstrapRoots > 0
      ? `${analysis.unresolvedBootstrapRoots} NestJS bootstrap root ${analysis.unresolvedBootstrapRoots === 1 ? "reference" : "references"} could not be statically resolved`
      : undefined,
    analysis.unresolvedImperativeGlobalGuards > 0
      ? `${analysis.unresolvedImperativeGlobalGuards} NestJS imperative global guard ${analysis.unresolvedImperativeGlobalGuards === 1 ? "registration" : "registrations"} could not be statically scoped`
      : undefined,
    analysis.unresolvedNestRoutingConfigurations > 0
      ? `${analysis.unresolvedNestRoutingConfigurations} NestJS application routing configuration ${analysis.unresolvedNestRoutingConfigurations === 1 ? "site" : "sites"} could not be statically scoped`
      : undefined,
    analysis.unresolvedNestRouterRegistrations > 0
      ? `${analysis.unresolvedNestRouterRegistrations} NestJS RouterModule ${analysis.unresolvedNestRouterRegistrations === 1 ? "registration" : "registrations"} could not be statically resolved`
      : undefined,
