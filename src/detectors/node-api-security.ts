...
export async function runNodeApiSecurity(context: ScanContext): Promise<DetectorResult> {
  ...
  const reasons = [
    `${detected} route and authorization coverage is bounded static inference; package aliases, dynamic registrations outside the supported local-constant forEach form, framework extensions, dynamic module metadata and provider factories without a statically visible local class result, local call chains beyond the four-edge traversal limit, dynamic/interpolated/disjunctive QueryBuilder predicates, computed parameter maps, general policy control flow, complex ORM wrappers, and external policy engines may require manual review`,
