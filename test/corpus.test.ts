...
test("the public corpus gives every native Beta rule a positive and near-miss with the expected evidence level", async () => {
  ...
  assert.deepEqual(result.catalog, { totalRules: 31, rulesWithPositive: 31, rulesWithNearMiss: 31 });
  assert.equal(result.totals.truePositive, 31);
  ...
  assert.equal(result.cases.filter((item) => item.variant === "positive").length, 11);
  assert.equal(result.cases.filter((item) => item.variant === "near_miss").length, 11);
});

test("the corpus catalog cannot drift from rule ids declared by native detectors", async () => {
  const detectorFiles = ["app-config.ts", "artifacts.ts", "baas.ts", "platform.ts", "secrets.ts", "typescript-dataflow.ts"];
