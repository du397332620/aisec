...
这还不是公开发布就绪状态。P0-01 至 P0-05 和 P0-07 已关闭；P0-06 的确定性产物、SBOM、校验清单和 GitHub Sigstore 证明已实现并通过无标签演练，正式版本标签、GitHub Release 与 npm 发布仍未执行。GitHub 私密漏洞报告已启用，安全政策写明支持版本、响应时限和协调披露流程。当前最大的证据缺口转为：语料仍是小型合成集合，尚不能代表真实世界效能；规模/恶意输入基准不足，npm 包身份也尚未决定。
...
1. 完成规模与恶意输入基准（P0-08）。
2. 确认 npm 包身份，执行首个正式标签/Release（P0-06）。
3. 在全新环境完成公开 Beta 文档走查（P0-09）。
