...
| FR-03 | Supabase/Firebase 授权配置检查 | P0 | 已完成 | Supabase/Firebase 均有独立阳性与近似阴性语料；本轮修复 `.rules` 文件未进入盘点导致的 Firebase 漏报 |
...
| FR-15 | CI、依赖审计和发布包检查 | P0 | 已完成 | Ubuntu 24.04 x64 / macOS 15 arm64 × Node.js 22/24 的四组合工作流和真实 tarball 安装 smoke 全部远程通过 |
...
| NFR-06 | 对大型或恶意仓库有已量化性能边界 | 已完成 | 500/5,000 文件和强制截断场景在独立进程记录耗时/RSS并纳入四组合 CI；文件/字节/发现/产物/子进程输出有硬上限，危险与超大归档、发现洪泛、共享输出预算均有 fail-closed 对抗测试 |
...
| 检查 | 结果 | 结论 |
| --- | --- | --- |
| `npm test` | 40 个测试；本地受限沙箱 39 通过 / 0 失败 / 1 跳过 | 唯一本地跳过项是执行沙箱禁止回环监听；新增参数硬上限、字节/目录条目截断、发现洪泛、共享子进程输出预算及危险/超大归档回归 |
...
| M1 Beta 基础 `0.1.0` | 形成可运行 CLI/MCP、原生规则、报告和安全执行边界 | 已完成 | 当前默认套件 40 个测试、真实引擎套件 2 个测试，打包主路径可用 |
...
| 合成语料造成虚高指标 | 误判检测质量 | 31 条规则均有独立阳性/近似阴性、按类别输出和目录漂移守卫，基准自带警告 | 引入脱敏真实项目样本与外部审阅，不使用总分掩盖薄弱类别 |
| 第三方解析器处理恶意仓库时存在漏洞 | 扫描器自身被攻击 | 不执行项目、子进程限制、配置隔离、危险/超大归档和输出洪泛对抗测试 | 继续建议 OS 沙箱；跟踪上游解析器安全公告并将公开 PoC 脱敏后转为回归 |
...
| D-06 | 漏洞响应负责人和私密联系方式 | 已决定由当前仓库维护者通过 GitHub Private Vulnerability Reporting 接收和协调；不公开个人邮箱 |
...
- 完成 P0-07：启用 GitHub Private Vulnerability Reporting，不公开个人邮箱；`SECURITY.md` 增加当前支持目标、私密报告入口、3/7 个工作日响应目标、每 14 天状态更新、协调披露和善意研究边界。
...
- 将 lockfile 中残留的第三方镜像下载地址统一为 npm 官方 registry；GitHub Actions run `31579898657` 的四个 OS/Node job 全部成功且 annotations 为 0，P0-05 已关闭。
...
- 扩展真实引擎语料，逐条验收随包发布的 3 条 Opengrep 规则（动态代码执行、不安全 pickle、弱哈希）及对应安全近似样本。
- 归一化 Opengrep 为内置规则附加的配置路径前缀，保证规则 ID、基线指纹和抑制键不随源码或安装包布局变化，并加入单元及真实引擎回归。
