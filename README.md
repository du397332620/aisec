...
project-local call chain; fixed arguments, conditional aliases, dynamic object
dispatch and ambiguous imports are not route-attributed. CORS, raw exception
...
| Python API data flow | Native Python analyzer | Bounded interprocedural traces for request-derived URL, file-path and raw-SQL sinks, plus caller-selected model origins receiving server credentials; exact FastAPI route origins propagate only through direct handlers and uniquely resolved local/relative-import calls, while ambiguous dispatch remains unattributed; recognizes selected validation boundaries and reports partial coverage |
